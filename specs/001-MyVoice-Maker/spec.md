...
```mermaid
flowchart TD
    A[録音ボタン] --> B[自分の声を3-300秒録音]
    B --> C[src/voice/models/samples/保存]
    D[原稿CSV入力] --> E[input/原稿.csv上書き]
    E --> F[音声生成ボタン
    output/tempクリア→音声保存]
    C --> F
    F --> G[output/temp/slide_*.mp3]
    G --> H[音声出力ダウンロード]
```
...
ユーザーは自分の声を3-300秒間録音し、話者サンプルとして保存できる。
...
ユーザーは原稿CSVから音声を生成でき、長文の場合も自動的に分割処理されタイムアウトを回避できる。

**この優先度の理由**: 長文音声生成時のタイムアウトエラーを防ぐため最優先。

**独立テスト**: 500文字以上の原稿を含むCSVをアップロードし、音声生成が120秒以内に完了することを確認する。

**受入シナリオ**:

| 前提条件 | 操作 | 期待結果 |
|----------|------|----------|
| 長文（500文字以上）を含むCSV | 音声生成ボタンをクリック | 自動分割され、タイムアウトなく生成完了 |
...
- 録音時間が範囲外（3秒未満、300秒超）の場合、デフォルト10秒にフォールバック
- tempフォルダ削除中にファイルがロックされている場合、エラーログを出力して続行（最大3回リトライ）
- 500文字を超える長文の場合、自動的に分割して音声生成
...
| ID | 要件 | 優先度 |
|----|------|--------|
| FR-001 | システムは3-300秒の録音時間設定を提供しなければならない | P1 |
...
| FR-009 | システムは500文字以上のテキストを自動分割して音声生成しなければならない | P1 |
| FR-010 | システムは音声生成のタイムアウトを120秒に設定しなければならない | P1 |
